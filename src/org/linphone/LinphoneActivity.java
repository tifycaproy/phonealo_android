...
import org.linphone.fragments.saldo.BalanceFragment;
...
public class LinphoneActivity extends Activity implements OnClickListener, ContactPicked
	...
	private RelativeLayout contacts, history, dialer, balance;
	private View contacts_selected, history_selected, dialer_selected, balance_selected;
...
//			finish();
			return;
		} else if (savedInstanceState == null && ApplicationConfig.getCurrentUser() == null) {
...
//		initSideMenu();
		...
		mUser = Data.getUser();
		new BalanceRestApiUtil().getBalance(mUser, this);
	...
	private void initButtons() {
		...
		balance_selected = findViewById(R.id.balance_select);
	...
	private void changeFragment(Fragment newFragment, FragmentsAvailable newFragmentType, boolean withoutAnimation) {
		...
		if (newFragmentType != FragmentsAvailable.DIALER
				...
				&& newFragmentType != FragmentsAvailable.BALANCE
...
//	}
	...
	public void displayChat(String sipUri) {
		...
		if (currentFragment == FragmentsAvailable.BALANCE || currentFragment == FragmentsAvailable.CHAT) {
			Fragment fragment2 = getFragmentManager().findFragmentById(R.id.fragmentContainer2);
			...
			if(isTablet()){
				changeCurrentFragment(FragmentsAvailable.BALANCE, null);
				displayChat(sipUri);
			} else {
				Bundle extras = new Bundle();
				extras.putString("SipUri", sipUri);
	...
	public void onClick(View v) {
		...
		} else if (id == R.id.balance || id == R.id.info_saldo) {
			changeCurrentFragment(FragmentsAvailable.BALANCE, null);
			balance_selected.setVisibility(View.VISIBLE);
	...
	private void resetSelection() {
		...
		balance_selected.setVisibility(View.GONE);
	}
...
//		changeCurrentFragment(FragmentsAvailable.DIALER, extras);
		...
		String numberFix = number.replaceAll("\\s+","");
...
//		result = StringUtil.loadJSONFromAsset(this, R.raw.balance);
	...
	public void loadInfoSaldo() {
		if(mUser.getUserBalance().getCurrency().equals("EUR")) {
			ApplicationConfig.currency_char = getString(R.string.euro_char);
		}else {
			ApplicationConfig.currency_char = getString(R.string.dollar_char);
		}

		TextView infoSaldo = (TextView) statusFragment.getView().findViewById(R.id.info_saldo);
		infoSaldo.setOnClickListener(this);
		
		//muestra en la barra superior de la pantalla el saldo y la moneda
		infoSaldo.setText(mUser.getUserBalance().getAmount() + " " + ApplicationConfig.currency_char);

		TextView balanceChar = (TextView) findViewById(R.id.balance_char);
		balanceChar.setText(ApplicationConfig.currency_char);


		if(mUser.getUserBalance().getAmount().equals("0")) {
			Toast.makeText(this, "Aviso! Tu saldo está agotado", Toast.LENGTH_LONG).show();
		}
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == Activity.RESULT_FIRST_USER && requestCode == SETTINGS_ACTIVITY) {
			...
			if(chat){
				pendingFragmentTransaction = FragmentsAvailable.BALANCE;
			}
	...
	public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
		// TODO: Comprobar que 'permissions' no sea nulo.
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			if (currentFragment == FragmentsAvailable.DIALER
					...
					|| currentFragment == FragmentsAvailable.BALANCE) {
